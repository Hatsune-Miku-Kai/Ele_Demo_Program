...
void Jog_Select_Move::Handle_Button()
{
    ...
    if(btn == BTN3)
    {
        ...
        switch(selectedIndex)
        {
            case 0://坐标自由移动
            {
                Coord_Select_Move* coord_select_move = new Coord_Select_Move(tft, button);
                ui_manager.RegisterScreen(coord_select_move);//将页面放入注册列表
                button.Wait();
                ui_manager.Change_UI(coord_select_move, true);
                break;
            }

            case 1://角度自由移动
            {
                Angle_Select_Move* angle_select_move = new Angle_Select_Move(tft, button);
                ui_manager.RegisterScreen(angle_select_move);//将页面放入注册列表
                button.Wait();
                ui_manager.Change_UI(angle_select_move, true);
                break;
            }
