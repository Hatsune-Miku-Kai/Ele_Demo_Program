...
Calibration::Calibration(TFT_eSPI &tft, Button &button) : Screen_Base(tft, button)
{
    id = ScreenID::ScreenID_Calibration;
}
...
void Calibration::Draw_Static()
{
    Draw_UI();
}

void Calibration::Draw_Update()
{
    Update_UI();
}

void Calibration::Handle_Button()
{
    ...
    if (btn == BTN3)
    {
        ...
        case 0:
        {  
            Automatic_Calibration* automatic_calibration = new Automatic_Calibration(tft, button);
            ui_manager.RegisterScreen(automatic_calibration);
            button.Wait();
            ui_manager.Change_UI(automatic_calibration, true);
            break;
        }        
        case 1:
        {
