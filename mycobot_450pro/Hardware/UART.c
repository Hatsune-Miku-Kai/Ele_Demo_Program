...
{
if (USART_GetFlagStatus(USART2, USART_FLAG_IDLE) == SET) // 检查空闲中断标志
    {
		...
		// 获取接收到的数据长度
		len = 1024 - DMA_GetCurrDataCounter(DMA1_Channel6);
		printf("%d",len);
		for(int i = 0; i< len ;i++)
...
/**
 * @brief 配置DMA通道用于USART2发送
 * 
 * 配置DMA1通道7用于USART2的发送功能。设置DMA的外设地址为USART2的数据寄存器，
 * 并配置为内存到外设的传输模式。
 */
...
//    NVIC_InitTypeDef NVIC_InitStructure;
    ...
    // 2. 配置 DMA 通道 7（对应 USART2_TX）
    DMA_DeInit(DMA1_Channel7);
    DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)&USART2->DR; // USART2 DR 地址
 ...
 * @retval 无
 */
void MODBUS_USART_ModeConfig(void)
{
...
void UART2_Send_Data_DMA(uint8_t *buf, uint16_t len)
{
    ...
    // 使能 DMA 通道
    DMA_Cmd(DMA1_Channel7, ENABLE);
	while (DMA_GetFlagStatus(DMA1_FLAG_TC7) == RESET);	//等待DMA工作完成
...
void MODBUS_Send_Data()
{
	if(read_flag)
	{
		...
		UART2_Send_Data_DMA(recv_buffer, send_buffer_len);
