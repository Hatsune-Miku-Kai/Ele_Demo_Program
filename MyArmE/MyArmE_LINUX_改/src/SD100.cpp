...
uint8_t Serial_Send_Buffer_CAN_SD[30] = {0x55, 0xAA, 0x1E, 0x01, 0X01, 0x00, 0x00, 0x00, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x01, 0x01, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0xFF,
                                      0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFC, 0x88}; // 13                                            //21



void SD100::Init_Serial(const char *Port)
{
...
void SD100::Set_Pos(uint8_t ID, int32_t len, uint32_t speed, uint32_t acc, uint32_t dec)
{
    ...
    Serial_Send_Buffer_CAN_SD[13] = frame_id;
    Serial_Send_Buffer_CAN_SD[14] = frame_id >> 8;
    ...
    for (int i = 0; i < 8; i++)
    {
        Serial_Send_Buffer_CAN_SD[i + 21] = Set_Pos_Array[i];
    }

    serial.writeBytes(Serial_Send_Buffer_CAN_SD, sizeof(Serial_Send_Buffer_CAN_SD));

    // for (int i = 0; i < sizeof(Serial_Send_Buffer_CAN_SD); i++)
    // {
    //     std::cout << std::hex;
    //     std::cout << std::setw(2) << std::setfill('0') << static_cast<int>(Serial_Send_Buffer_CAN_SD[i]) << " ";
    // }
    // std::cout << std::endl;
...
{
    uint16_t id = (0x06 << 8) | ID;
    Serial_Send_Buffer_CAN_SD[13] = id;
    Serial_Send_Buffer_CAN_SD[14] = id >> 8;

    for (int i = 0; i < 8; i++)
    {
        Serial_Send_Buffer_CAN_SD[i + 21] = Power_On_Array[i];
    }

    Serial_Send_Buffer_CAN_SD[25] = 0x06;
    serial.writeBytes(Serial_Send_Buffer_CAN_SD, sizeof(Serial_Send_Buffer_CAN_SD));
    usleep(800);

    Serial_Send_Buffer_CAN_SD[25] = 0x07;
    serial.writeBytes(Serial_Send_Buffer_CAN_SD, sizeof(Serial_Send_Buffer_CAN_SD));
    usleep(800);
    Serial_Send_Buffer_CAN_SD[25] = 0x0F;
    serial.writeBytes(Serial_Send_Buffer_CAN_SD, sizeof(Serial_Send_Buffer_CAN_SD));
    usleep(800);
...
int32_t SD100::Read_Pos(uint8_t ID)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        Serial_Send_Buffer_CAN_SD[i + 21] = Read_Pos_Array[i];
    }
    ...
    while(true)
    {
        int byte_num = serial.readBytes(Recv_Buffer, 32);
...
void SD100::Set_Speed(uint8_t ID, uint32_t speed)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        Serial_Send_Buffer_CAN_SD[i + 21] = Set_Speed_Array[i];
    }
    serial.writeBytes(Serial_Send_Buffer_CAN_SD, sizeof(Serial_Send_Buffer_CAN_SD));
...
void SD100::Set_Acc(uint8_t ID, uint32_t acc)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        Serial_Send_Buffer_CAN_SD[i + 21] = Set_Acc_Array[i];
    }
    serial.writeBytes(Serial_Send_Buffer_CAN_SD, sizeof(Serial_Send_Buffer_CAN_SD));
...
void SD100::Set_Dec(uint8_t ID, uint32_t dec)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        Serial_Send_Buffer_CAN_SD[i + 21] = Set_Dec_Array[i];
    }
    serial.writeBytes(Serial_Send_Buffer_CAN_SD, sizeof(Serial_Send_Buffer_CAN_SD));
...

{
    ...
    for (int i = 0; i < 8; i++)
    {
        Serial_Send_Buffer_CAN_SD[i + 21] = Set_Pos_Mode_Array[i];
    }
    serial.writeBytes(Serial_Send_Buffer_CAN_SD, sizeof(Serial_Send_Buffer_CAN_SD));
...
void SD100::Run(uint8_t ID)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        Serial_Send_Buffer_CAN_SD[i + 21] = Run_Array[i];
    }
    serial.writeBytes(Serial_Send_Buffer_CAN_SD, sizeof(Serial_Send_Buffer_CAN_SD));
...
void SD100::Stop(uint8_t ID)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        Serial_Send_Buffer_CAN_SD[i + 21] = Stop_Array[i];
    }
    serial.writeBytes(Serial_Send_Buffer_CAN_SD, sizeof(Serial_Send_Buffer_CAN_SD));
    usleep(800);
}
...
void SD100::Set_Calibration(uint8_t ID)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        Serial_Send_Buffer_CAN_SD[i + 21] = Set_Calibration_Array[i];
    }
    serial.writeBytes(Serial_Send_Buffer_CAN_SD, sizeof(Serial_Send_Buffer_CAN_SD));
