...
void MotorMT ::Set_Motor_Angles(uint8_t *ID, uint8_t id_len, float *angles, int speed)
{
    ...
    if(is_exceed_angles == 0)
    {
        ...
        for (; i < id_len; i++)
        {
            ...
            int16_t temp =((Read_Buffer.second[3] << 8 | Read_Buffer.second[2]));
            temp = (temp / 1.414) * 1.9;
            liju1 = (float)temp;
            printf("%0.8f\n\r",temp);
        }  
...
Angles MotorMT::Get_Motor_Angles(uint8_t *ID, uint8_t id_len)
{
    ...
    for (; i < id_len; i++)
    {
        ...
        // printf("%x\n\r ",Read_Buffer.second[5]);

        // printf("%x\n\r ",Read_Buffer.second[4]);
...
/**
 * @brief 修改电机的PID参数(PID均默认为1)
 * @param ID 电机ID数组指针(1~6),若测试单电机,代码可参考如下
 *                                                          uint8_t ID[] = {x};//x为需要测试的单电机ID号
 *                                                          MT.Change_Motor_PID(ID,1)//表示此时传入的ID数组只有一个电机
 *                                                          并且需要控制的电机数目也为1,后续参数可依照需求调整
 * @param id_len 电机数量
 * @param New_P 新的比例参数
 * @param New_I 新的积分参数
 * @param New_D 新的微分参数
...
uint8_t MotorMT::Read_Motor_ID()
{
    ...
    send_buffer[2] = 0x01;
    CAN.SendMsg(send_buffer,8,0x300);
    CAN.ReadMsg(send_buffer, 8, 0x300, Read_Buffer, false);
...
{
    std::pair<int, vector<uint8_t>> Read_Buffer;
    Read_Buffer.second.resize(5);
    uint8_t send_buffer[4] = {0xff,0xff,0x00,0x82};
    CAN.ReadMsg(send_buffer, 4, 0x7FF, Read_Buffer, false);
    for(int i = 0; i < Read_Buffer.second.size(); i++)
    {
        printf("%02x ",Read_Buffer.second[i]);
    }
    printf("\n\r");
...
void MotorMT::Encous_Set_Zero()
{
    std::pair<int, vector<uint8_t>> Read_Buffer;
    Read_Buffer.second.resize(8);
    uint8_t send_buffer[4] = {0x00,0x01,0x00,0x03};
    CAN.ReadMsg(send_buffer, 4, 0x7FF, Read_Buffer, false);
    for(int i = 0; i < Read_Buffer.second.size(); i++)
    {
        printf("%02x ",Read_Buffer.second[i]);
...
void MotorMT::Encous_Stop()
{
    std::pair<int, vector<uint8_t>> Read_Buffer;
    Read_Buffer.second.resize(8);
    uint8_t send_buffer[3] = {0x60, 0x7F, 0xFF};
    CAN.SendMsg(send_buffer, 3, 0x01);
}

void MotorMT::Encous_Set_Angle()
{
    std::pair<int, vector<uint8_t>> Read_Buffer;
    Read_Buffer.second.resize(8);
    uint8_t send_buffer[8] = {0x03 ,0x40 ,0x00 ,0x00 ,0x01 ,0x2C ,0x03 ,0xE9};
    CAN.ReadMsg(send_buffer, 8, 0x01, Read_Buffer, false);
    for(int i = 0; i < Read_Buffer.second.size(); i++)
    {
        printf("%02x ",Read_Buffer.second[i]);
    }
...
void MotorMT::Encous_Reset()
{
    std::pair<int, vector<uint8_t>> Read_Buffer;
    Read_Buffer.second.resize(8);
    uint8_t send_buffer[8] = {0x20, 0x00, 0x00, 0x00, 0x00 ,0x00 ,0x00 ,0x00};
    CAN.ReadMsg(send_buffer, 8, 0x01, Read_Buffer, false);
    for(int i = 0; i < Read_Buffer.second.size(); i++)
    {
        printf("%02x ",Read_Buffer.second[i]);
