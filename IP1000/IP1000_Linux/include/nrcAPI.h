...
{
	int syncGroupSum; //每个机器人外部轴组最大数目，最大为3,最多支持两个双变位机加一个地轨
	std::vector<int> syncType; //外部轴组类型1:单轴变位机，2:双轴变位机，3:地轨
	std::vector<std::vector<int>> syncServoVec; //外部轴伺服映射关系
};
...
{
	double headIndent = 0;  //头部缩进
	double tailIndent = 0;  //尾部缩进
	double leftRightOffset = 0;  //左右偏移
	double upDownOffset = 0;	//高低推角
	double pushAngle = 0;  //推角
	double tiltAngle = 0;  //倾斜角
	int weldDir = 0;  //焊接方向
	bool headMultiIndent = false;  //头部倍数缩进使能
	bool tailMultiIndent = false;  //尾部倍数缩进使能
};
...
struct NRC_Variant
{
	...
	double varValue = 0;  //valueType类型为Data 时的参数值
...
{
	int pistolSwitch = 0;			//焊枪开关
	int arcingSuccess = 0;			//引弧成功
	int handWireFeed = 0;			//手动送丝
	double weldCurrent = 0;		//焊接电流
	double weldVoltage = 0;		//焊接电压
	double runTime = 0;			//焊接时间
	double dutyRatio = 0;			//焊接占空比
};
...
/**
 * @brief 将机器人直角坐标值转换为关节坐标值(逆解参考点为零点)
...
/**
 * @brief 根据IO板号，数字IO端口输出
 * @param board_id 要输出的IO板号，最大范围为 1 <= board_id <= 4，实际范围取决于所连接的IO板数量
 * @param port_id 要输出的IO板号所对应的数字IO端口，最大范围为 1 <= port <= 16，实际范围取决于该IO板号所对应IO版的IO端口数目
 * @param value 要输出的状态
 *    -   0  输出低电平
 *    -   1  输出高电平
 */
...
/**
 * @brief 根据IO板号，读取数字IO输出端口的状态
 * @param board_id 要读取的IO板号，最大范围为 1 <= board_id <= 4，实际范围取决于所连接的IO板数量
 * @param port_id 要读取的IO板号所对应的数字IO端口，最大范围为 1 <= port <= 16，实际范围取决于该IO板号所对应IO版的IO端口数目
 * @return 返回该端口当前状态
 * @retval  0  当前处于低电平
 * @retval  1  当前处于高电平
 * @retval  -1  该端口不存在
...
/**
 * @brief 当前打开的作业文件的总行数
 * @pre 调用该函数前，请先调用 NRC_CreateJobfile(const std::string &jobname) 或 NRC_OpenJobfile(const std::string &jobname) 创建或打开一个作业文件
...
/**
 * @brief 停止运行作业文件
 * @note 停止运行作业文件，执行后再调用 NRC_StartRunJobfile(const std::string &jobname) 将从头开始运行
 */
int NRC_StopRunJobfile();

/**
 * @brief 获取当前位置距离原插补轨迹停止点的距离
 * @param distance 停止点和当前点位的直线距离
 */
int NRC_GetDistanceToOldTrack(int robot, double &distance);

/**
 * @brief 停止运行作业文件并且不下使能
 * @note 停止运行作业文件，并且不会下使能，执行后再调用 NRC_StartRunJobfile(const std::string &jobname) 将从头开始运行
 */
int NRC_StopRunJobfileNotPoweroff();

/**
 * @brief 获取当前程序执行方向(顺序执行还是倒叙执行)
 * @param order 执行方向 true/false(顺序执行/倒叙执行)
 */
 ...
 * @param target 机器人运动目标位置，详见 NRC_Position
 * @param pl 平滑度，将和后面一条移动指令进行平滑过渡，数值越大，越平滑，轨迹偏差也越大，参数范围：0 <= pl <= 5
 * @param ty 提前执行类型, 0:Time, 1:Distance 2:Proportion
 * @param vlu 提前执行参数值
 */
...
/**
 * @brief 作业文件插入MOVL指令,点位使用全局点位
 * @pre 调用该函数前，请先调用 NRC_CreateJobfile(const std::string &jobname) 或 NRC_OpenJobfile(const std::string &jobname) 创建或打开一个作业文件
 * @param line 将指令插入到第line行，参数范围：0 < line < (NRC_GetJobfileLineSum()+1)
 * @param vel 机器人的运行速度，为机器人末端位置点绝对运行速度，单位为 毫米每秒（mm/s），参数范围：vel > 1
 * @param acc 机器人运行加速度，为机器人笛卡尔最大加速度的百分比，参数范围：0 < vel <= 100
 * @param dec 机器人运行减速度，为机器人笛卡尔最大减速度的百分比，参数范围：0 < vel <= 100
 * @param posName 全局点位名 例如"GP0001"
 * @param pl 平滑度，将和后面一条移动指令进行平滑过渡，数值越大，越平滑，轨迹偏差也越大，参数范围：0 <= pl <= 5
 * @param ty 提前执行类型, 0:Time, 1:Distance 2:Proportion
 * @param vlu 提前执行参数值
 */
 ...
 * @param pl 平滑度，将和后面一条移动指令进行平滑过渡，数值越大，越平滑，轨迹偏差也越大，参数范围：0 <= pl <= 5
 * @param ty 提前执行类型, 0:Time, 1:Distance 2:Proportion（关节运动仅支持时间）
 * @param vlu 提前执行参数值
 */
 ...
 * 		- 值为0时，将无限时间等待，直到条件成立
 *@param  ret  返回值写入的bool变量 1为条件达成 0为其他
 */
 ...
 * @brief 作业文件插入ADD指令
 * @param line 将指令插入到第line行，参数范围：0 < line < (NRC_GetJobfileLineSum()+1)
 * @param lValue 求出的和存储到哪个变量中，例如存到“I001”
 * @param rValue1 加数
 * @param rvalue2 加数
...
/**
 * @brief 作业文件插入TOFFSETON指令
 * @pre 调用该函数前，请先调用 NRC_CreateJobfile(std::string jobname) 或 NRC_OpenJobfile(std::string jobname) 创建或打开一个作业文件
 * @param line 将指令插入到第line行，参数范围：0 < line < (NRC_GetJobfileLineSum()+1)
 * @param data 偏移量，详见 NRC_Position
 * @attention 需要和 NRC_JobfileInsertTOFFSETOFF(int line) 配合使用
 */
int NRC_JobfileInsertTOFFSETON(int line, const NRC_Position& data);
/**
 * @brief 运行队列插入TOFFSETON指令
 * @param 偏移量，详见 NRC_Positio
 * @attention 需要和 NRC_RunqueueInsertTOFFSETOFF() 配合使用
 */
int NRC_RunqueueInsertTOFFSETON(const NRC_Position& data);
/**
 * @brief 作业文件插入TOFFSETOFF指令
 * @pre 调用该函数前，请先调用 NRC_CreateJobfile(std::string jobname) 或 NRC_OpenJobfile(std::string jobname) 创建或打开一个作业文件
 * @param line 将指令插入到第line行，参数范围：0 < line < (NRC_GetJobfileLineSum()+1)
 * @attention 需要和 NRC_JobfileInsertTOFFSETON(int line, const NRC_Position& data) 配合使用
 */
int NRC_JobfileInsertTOFFSETOFF(int line);
/**
 * @brief 运行队列插入TOFFSETOFF指令
 * @attention 需要和 NRC_RunqueueInsertTOFFSETON(const NRC_Position& data) 配合使用
 */
 ...
 * @brief 运行队列插入ARCON指令
 * @pre 调用该函数前，请先调用 NRC_CreateNoFlieRunqueue() 创建一个无文件运行队列
 * @param id 焊接工艺的工艺号，参数范围：1 <= id <= 99
 ...
 * @brief 作业文件插入ARCON指令
 * @param id 焊接工艺的工艺号，参数范围：1 <= id <= 99
 * @attention 需要和 NRC_JobfileInsertARCOFF(int id) 配合使用
 */
...
/**
 * @brief 作业文件插入WVON指令
 * @param id 摆焊工艺的工艺号，参数范围：1 <= id <= 99
 * @attention 需要和 NRC_JobfileInsertWVOFF(int line, int id) 配合使用
 */
int NRC_JobfileInsertWVON(int line, int id);
/**
 * @brief 作业文件插入WVOFF指令
 * @param id 摆焊工艺的工艺号，参数范围：1 <= id <= 99
 * @attention 需要和 NRC_JobfileInsertWVON(int line, int id) 配合使用
 */
int NRC_JobfileInsertWVOFF(int line, int id);
/**
 * @brief 作业文件插入ARCTRACKON指令
 * @param id 电弧跟踪的工艺号，参数范围：1 <= id <= 99
 * @attention 需要和 NRC_JobfileInsertARCTRACKOFF(int line, int id) 配合使用
 */
int NRC_JobfileInsertARCTRACKON(int line, int id);
/**
 * @brief 作业文件插入ARCTRACKOFF指令
 * @param id 电弧跟踪的工艺号，参数范围：1 <= id <= 99
 * @attention 需要和 NRC_JobfileInsertARCTRACKON(int line, int id) 配合使用
 */
int NRC_JobfileInsertARCTRACKOFF(int line, int id);
/**
 * @brief 作业文件插入SEARCH_STATIC指令
 * @param fileNum 寻位文件号，参数范围：1 <= id <= 99
 * @param tableNum 寻位参数表号，参数范围：1 <= id <= 99
 * @param posName 输出位置
...
/**
 * @brief 作业文件插入MP_OFFSET_ON指令
 * @param id 工艺号
 * @param traceData 路径数据
 * @param offsetPass 焊道数
 * @attention 需要和 NRC_JobfileInsertMP_OFFSET_OFF(int line) 配合使用
 */
int NRC_JobfileInsertMP_OFFSET_ON(int line, int id,const NRC_Variant& traceData,const NRC_Variant& offsetPass);

/**
 * @brief 作业文件插入MP_OFFSET_OFF指令
 * @attention 需要和 NRC_JobfileInsertMP_OFFSET_ON(int line) 配合使用
 */
int NRC_JobfileInsertMP_OFFSET_OFF(int line);

/**
 * @brief 获得当前作业文件局部点位的数量
 */
...
/**
 * @brief 设置伺服映射关系 多机版
 * @param robotSum 机器人总数,最大为4
 * @param robotType 每个机器人的名字(类型参考NRC_Rbt_SetRobotTypeConfig函数);
 * @param servoMap 伺服映射关系，个数为设置的机器人总数，值为每一个机器人轴所连接的伺服编号
 * @par 说明:
...
/**
 * @brief 通过从站vendorID和productCode获取从站的编号列表
 * @param vendorID 从站的vendorID
 * @param productCode 从站的productCode
 * @return 所有该类型从站的编号的列表
 */
 ...
 * @param slaveNum 从站编号
 * @param index PDO的索引
 * @return PDO的映射地址，若从站编号错误或无该PDO，则返回nullptr，若该索引的PDO存在多个，将只会返回1个
 * @attention 返回的映射地址，需要根据PDO的数据类型，自行转换指针类型后才能使用
 */
 ...
 * @param slaveNum 从站编号
 * @param index PDO的索引
 * @param subindex PDO的子索引
 * @return PDO的映射地址，若从站编号错误或无该PDO，则返回nullptr
 * @attention 返回的映射地址，需要根据PDO的数据类型，自行转换指针类型后才能使用
 */
...
/**
 * @brief 使能自定义IO功能
 * @attention 请在调用控制器启动函数 NRC_StartController 前调用使能,否则将不生效
 */
 ...
 * @param enable 使能机器人范围，参数范围：0 关闭，１使能
 * @param robotRangeNum 机器人范围编号，参数范围：0 < robotRangeNum < 100
 * @param workType 机器人范围生效方式，参数范围：0 输出状态，１触发停止
 * @param isInInterfer 生效范围，参数范围：true 设置范围内生效，false 设置范围外生效
 * @param port 输出io端口号，仅在workType为０时有效
 * @param value 输出io高低电平，参数范围：0 低电平，１高电平
 * @attention 请在伺服停止或伺服就绪状态下设置该参数
 */
...
/**
 * @brief 将当前位置设置为零点或清除多圈值(多机版)
 * @param robotNum 需要设置零点或清空多圈值的机器人编号
 * @param axisNum 操作的轴的序号，当axisNum为0时，表示全部轴，参数范围：0 <= axisNum <= 机器人轴总数
 * @param mod为设置零点或清除多圈值，0为设置零点，1为清除多圈值
 * @attention 请在伺服停止或伺服就绪状态下设置该参数
 * @attention 执行该函数大约需要两三秒时间
...
/**
 * @breif 设置零点位置单圈值(多机版)
 * @param robotNum 需要设置零点位置单圈值的机器人编号
 * @param value 传入各个轴零点位置单圈值数组，0 < value.size() <= 7
 * @attention 此操作有可能导致机器人精度丢失、无法正常运行、曾建立的点位无法运行等问题，请谨慎操作
 */
int NRC_Rbt_SetSingleCircleValue(int robotNum, const std::vector<int>& value);

/**
 * @breif 获取零点位置单圈值(多机版)
 * @param robotNum 需要获取零点位置单圈值的机器人编号
 * @param value 获取到的值存入此数组
 */
 ...
 * @param userNum 要设置的用户坐标系的编号，参数范围：0 < userNum < 1000
 ...
 * @param userNum 要获取的用户坐标系的编号，参数范围：0 < userNum < 1000
 * @param deviation 用户坐标系原点相对机器人直角坐标系原点的偏移量
 * 	- deviation.coord 该参数无用
 * 	- deviation.pos[0]-deviation.pos[2] 分别为x,y,z三轴位置的偏移量，单位：毫米
 * 	- deviation.pos[3]-deviation.pos[5] 分别为x,y,z三轴方向的偏移量，单位：弧度
...
/**
 * @brief 获取工具手参数
 * @param toolNum 要获取的工具手的编号，参数范围：0 < toolNum < 1000
 * @param deviation 要获取到的工具手参数,工具手末端位置相对机器人末端位置的偏移量
 * @param payload 要获取到的工具手负载质量等参数
 */
 ...
 * @param calibrationType 标定需要几个点，两点标定为2,20点为20,20点无零点为21
 * @param toolhandNumber  标定工具手编号
 * @par 示例:
 * @code
 ...
 * @param num 要获取的消息在队列中的编号，最早的消息编号为1，取值范围：0 < num <= NRC_GetMessageSize()
...
/**
 * @brief 指令TOFFSETON的数据类
* @attention 需要和 NRC_InstrDataTOFFSETOFF 配合使用
 */
...
/**
 * @brief 指令TOFFSETOFF的数据类
* @attention 需要和 NRC_InstrDataTOFFSETON 配合使用
 */
...
/**
 * @brief 断点模式暂停追加运行
 * @note 成功调用该函数后，机器人将暂停运行，并记录当前断点处运行状态，下电，该函数会在机器人停止后或调用500毫秒后退出,
 *       调用 NRC_RestartInstrAppendRunInBreakpoint() 可继续运行
 * @attention 暂停时添加的运行队列，不会立刻运行，而是放入缓存队列中，调用再启动后，才会运行该队列
 */
int NRC_PauseInstrAppendedRunInBreakpoint();
/**
 * @brief 断点模式重启追加运行
 * @note 在机器人断点停止时，调用该函数可使机器人继续运行
 */
...
/**
 * @brief 停止已追加的运行
 * @note 成功调用该函数后，机器人将停止运行，并清空全部运行队列，仍在追加模式，不下电
 * @attention 需要调用 NRC_RestartInstrAppendRun() 才能继续运行新追加队列
 */
...
/**
 * @brief 获取机器人外部轴组类型
 * @param robot 机器人号， 0< robot <= 4
 * @param sync_group_type 获取到的数据按照外部轴连接顺序存入sync_group_type容器
 * @note 外部轴各轴组类型，1:单轴变位机,2:双轴变位机,3:地轨
 */
int NRC_Rbt_GetSyncJointType(int robot, std::vector<int>& sync_group_type);

/**
 * @brief 设置外部轴组伺服映射关系
 * @param syncGroupSum 外部轴组最大数目，最大为12
 * @param syncType 外部轴组类型，1:旋转单轴，2:旋转双轴，3:直线单轴，4:直线双轴，5:直线三轴
 * @param syncServoMap 外部轴伺服映射关系
 * @par 说明:
 *   外部轴个数与伺服对应关系
 *     - 无外部轴，则 syncGroupSum 值为0,
 *     - 一个旋转单轴，则 syncGroupSum 值1,syncType为{1},syncServoMap为{{7}}
 *     - 两个旋转双轴，一个直线单轴，则 syncGroupSum 值3,syncType为{2,2,3},syncServoMap为{{7,8},{9,10},{11}}
 */
int NRC_SetSyncGroupAndAxisMap(unsigned int syncGroupSum, std::vector<int> syncType, std::vector<std::vector<int>> syncServoMap);

/**
 * @brief 设置机器人与外部轴组的绑定关系
 * @param syncGroupSum 要设置的外部轴轴组数，参数范围：0 <= syncGroupNum <= 3
 * @param syncGroupNum 要设置的外部轴各轴组编号
 * @attention 请在伺服停止或伺服就绪状态下设置该参数
 * @attention 修改后重启才能生效！！！
 */
int NRC_BindingRobotAndSyncGroup(unsigned int syncGroupSum, std::vector<int> syncGroupNum);
/**
 * @brief 设置机器人与外部轴组的绑定关系 多机版
 * @param syncGroupSum 要设置的各机器人的外部轴轴组数，参数范围：0 <= syncGroupNum <= 3
 * @param syncGroupNum 要设置的各机器人的外部轴各轴组编号
 * @attention 请在伺服停止或伺服就绪状态下设置该参数
 * @attention 修改后重启才能生效！！！
 */
...
/**
  * @brief 获取外部轴地轨转换比
  * @param xDirection   X方向
  * @param yDirection   Y方向
  * @param zDirection   Z方向
 */
...
/**
  * @brief 设置外部轴地轨是否协作
  * @param coopertion   是否协作
 */
int NRC_SetSyncCooperation(bool coopertion);

/**
  * @brief 获取外部轴地轨是否协作
  * @param syncGroupNum 要设置的轴组的序号，参数范围：1 <= syncGroupNum <= 3
 */
...
/**
  * @brief 标定外部轴点位(组号，点位编号)
  * @param syncGroupNum 要设置的轴组的序号，参数范围：1 <= syncGroupNum <= 3
  * @param pointNum 要设置的点位编号，0 <= pointNum <= 4
 */
 ...
 * @brief 设置机器人外部轴参数
 * @param syncGroupNum 要设置的轴组的序号，参数范围：1 <= syncGroupNum <= 3
 * @param syncAxisNum 要设置的某轴组中轴的序号，参数范围：1 <= syncAxisNum <= 2
 * @param config see NRC_RobotJointConfig
 * @attention 请在伺服停止或伺服就绪状态下设置该参数
 */
 ...
 * @brief 作业文件插入点位全改指令
 * @param line 将指令插入到第line行，参数范围：0 < line <= (NRC_GetJobfileLineSum()+1)
* @param pos_name 需要修改的全局点位名称，范围 "GE0001"~"GE9999"
 * @param change 详见 NRC_Position(NRC_COORD coo, int usr, int tool, int con,
 *	 	 	 	 	 	 	 	 	 double axis_pos1, double axis_pos2, double axis_pos3, double axis_pos4, double axis_pos5, double axis_pos6, double axis_pos7)
 * @param sync_change 详见 NRC_SyncPosition
 * @attention 注意，无论是 change 还是 sync_change，不存在的轴必需传入 99999
 */
...
/**
 * @brief 获取外部轴当前位置
 * @note 该函数可以获取外部轴位置，位置数据由参数引用 position 返回
 ...
 * @param (*fun)(int) 函数指针，即自定义指令要调用的函数
 */
int NRC_SetJobFileCustomInstructionCB(bool (*fun)(int,std::string,std::string));

/**
 * @brief 设置作业文件自定义指令预处理回调函数
 * @note 该函数的功能是将一个函数绑定到自定义指令的预处理函数中
 * @param (*fun)() 函数指针，即自定义指令要调用的预处理函数
 */
int NRC_SetJobFileCustomInstructionPreTreatmentCB(void (*fun)(int,std::string,std::string));

/**
 * @brief 设置作业文件自定义指令后处理回调函数
 * @note 该函数的功能是将一个函数绑定到自定义指令的后处理函数中
 * @param (*fun)() 函数指针，即自定义指令要调用的后处理函数
 */
...
/**
 * @brief 设置伺服运行模式
 * @param moderValue 伺服运行模式：3:速度模式PV,８:周期位置模式CSP, 9:周期同步速度模式(CSV), 10:周期同步转矩模式(CST)
 */
...
/**
 * @brief 设置一个自定义的无参函数，每个EtherCat通讯周期(默认1ms)回调一次
 * @param void (*fun)() 传入一个函数指针
 * @attention 回调函数中不得含有任何耗时操作
 */
 ...
 * @brief 设置目标速度值
 * @param axisNum 轴号,1为起始轴数
 * @param vel 传入一个double数组设置速度值(注:数组的大小需要和当前机器人轴数一致)
...
/**
 * @brief 获取规划队列的第一个点位
 * @param axisSum 机器人总轴数
 * @param axisPosition 返回获取点位的结果,数组纬度是axisSum
 */
 ...
 * @brief 设置目标速度值 多机版
 * @param robotNum 传入机器人编号(1-4)
 * @param vel 传入一个int数组设置速度值(注:数组大小需要和当前机器人轴数一致)
 * @如需使用该接口，eni文件必须要配PDO(0x60FF)才可以使用
 */
 ...
 * @brief 设置转矩补偿值 多机版
 * @param robotNum 传入机器人编号(1-4)
 * @param OffsetVal 传入各关节的转矩补偿值的数组(注:数组大小需要和当前机器人轴数一致)
 */
int NRC_Rbt_SetTorqueOffsetValue(int robotNum, signed short* OffsetVal);

/**
 * @brief 获取机器人数量
 * @param robotSum 传入一个int变量,以引用的方式返回当前的机器人数量;
 */
int NRC_GetRobotSum(int &robotSum);

/**
 * @brief 屏蔽示教器连接状态检测(需要在系统启动函数前调用)
 * @param status 传入的状态 true:屏蔽和示教器的通讯　false:打开和示教器的通讯
 */
...
/**
 * @brief 退出作业文件,通常用于外部结束作业文件，配合NRC_JobRun_GetInstructionAheadReturn()接口使用
 * 当此接口被调用后，NRC_JobRun_GetInstructionAheadReturn()的条件会被满足，此时用return退出函数即可
 * 此函数不会立刻停止作业文件，需要配合NRC_JobRun_GetInstructionAheadReturn()的检测
 */
...
/**
 * @brief 获取作业文件退出标志位，一般跟在NRC_JobRun_BlockNotMoveInstruction()接口或某些耗时操作后面使用，用于防止程序已停止而自定义指令函数没有正常退出
 * @brief 当作业文件重新运行或外部调用NRC_JobRun_QuitJob()函数时，上一次的未运行完成的作业文件需要通过此接口进行判断并用return退出，否则新作业文件指令无法正确执行
 * 例如，当自定义函数执行一个循环检测信号的功能时，每次循环除检测信号外，也需检测此接口，这样在未接收到信号时程序意外停止也能安全退出这个循环
 */
...
/**
 *	@brief 检查filepath路径下有没有这个文件，没有的话会调用createile回调，有的话会读取文件的内容，存到root中
 */
int NRC_CheckCreateReadJsonFile(void (*createFile) (const char*), const char *filepath, std::string &root);

/**
 *	@brief 保存root的内容到filepath这个文件，注意会覆盖掉原本的内容
 ...
 *	@brief 获取多重多道焊接参数
 *	@param craftNum 工艺号
 *	@param weldBeadCount 焊道数
 *	@param weldBeadParam 焊道具体参数，具体说明参见结构体定义
 */
int NRC_GetMultipassWeldParm(int craftNum,int& weldBeadCount,NRC_WeldBeadParam& weldBeadParam);

/**
 * @param robotNum 机器人编号1-4(此函数需要多机模式下才生效,设置多机模式函数NRC_SetRobotMode)
 * @brief 伺服上使能(多机版)
 * @pre 调用该函数前，请将伺服准备状态切换至伺服就绪，可调用 NRC_SetServoReadyStatus(int status) 函数切换伺服状态
 * @warning 成功调用该函数后，伺服将处于上电使能状态，请注意安全
 */
...
/**
 * @param robotNum 机器人编号1-4(此函数请在多机模式下使用,设置多机模式函数NRC_SetRobotMode)
 * @brief 伺服下使能(多机版)
 * @note 成功调用该函数后，伺服将下电，处于未使能状态
 */
...
/**
* @brief 获取机器人外部轴速度(多机版)
* @param robotNum 机器人编号1-4
* @param syncSpeed 以数组的方式返回当前点动的外部轴速度（数组大小为1-5)
*/
...
/**
 * @brief 计算特定坐标系下，点位进行固定距离偏移后的位置
 * @param robotNum 机器人编号1-4
 * @param pos 要进行偏移的点
 * @param deviation 偏移距离
 * @param coord 坐标系
 * @param resultPos 计算结果存储点位
 */
...
/**
 * @brief 送丝
 * @param stats 1:开始送丝  0：停止送丝
 */
int NRC_FeedWire(int stats);

/**
 * @brief 退丝
 * @param stats 1:开始退丝  0：停止退丝
 */
int NRC_ReturnWire(int stats);

/**
 * @brief 送气
 * @param stats 1:开始送气  0：停止送气
 */
